...
static int register_func(struct yz_func *self,
		struct parser *parser,
		enum YZ_SCOPE_TYPE scope_of);
...
int get_func_arg(struct yz_func *self, struct parser *parser)
{
	...
	if (get_ident_with_type(parser, &ident->name, &ident->type))
		ereturn(1, parser, "failed to get identifier with type");
	if (ident->type.type == YZ_VOID)
		ereturn(1, parser, "function argument type cannot be 'void'");

	ident->is_func_arg = true;

	assert(wrapper = append_symbol(&parser->symbols, &ident->name));
...
int get_func_args(struct yz_func *self, struct parser *parser)
{
	...
	if (CUR_TOK(parser).data.symbol == LEXER_SYM_COLON)
		return 0;
	if (!eat_tok_with_sym(LEXER_SYM_PAREN_L, parser))
		goto err_unexpected_tok;
	while ((ret = check_func_arg_end(parser)) != LIST_END) {
		if (ret == LIST_ERR)
			panic("failed to check function arg end");
		...
		uint16_t index)
{
	...
	if (CUR_TOK(parser).kind == SCLEXER_EOL)
		ereturnf(1, parser, "too few arguments for function '%s'",
				self->callee->name.s);

	if (!(val = parse_literal(parser)))
		ereturn(1, parser, "failed to parse function argument");

	if (!convert_type_implicity(&cur->type, &val->type))
		ereturnf(1, parser, "unsupport implicit convertions type "
				"'%s' and '%s'",
...
int get_func_name(struct parser *parser, str *result)
{
	...
	if (CUR_TOK(parser).kind != SCLEXER_IDENT)
		ereturn(1, parser, "miss function name");
		...
		enum YZ_SCOPE_TYPE scope_of)
{
	...
	panicf("failed to get scope '%d'", scope_of);
	return NULL;
}
...
int get_func_result_type(struct yz_func *self, struct parser *parser)
{
	if (!eat_tok_with_sym(LEXER_SYM_COLON, parser))
		ereturnf(1, parser, "miss function result type, "
				"unexpected token: '%s'",
				get_token_str(&CUR_TOK(parser)));
	if (CUR_TOK(parser).kind != SCLEXER_IDENT)
		return 1;
...
int register_func(struct yz_func *self,
		struct parser *parser,
		enum YZ_SCOPE_TYPE scope_of)
{
	...
	if (!wrapper)
		ereturnf(1, parser, "symbol '%s' is defined", self->name.s);
	wrapper->type = YZ_FUNC;
	wrapper->data.yz_func = self;
	debug_printf("function '%s:%s' defined",
			scope_of == YZ_SCOPE_PUBLIC ? "public" : "private",
			self->name.s);
	return 0;
}
...
int parse_func_def(struct parser *parser, enum YZ_SCOPE_TYPE scope_of)
{
	...
	if (register_func(self, parser, scope_of))
		goto err_free_self;
	...
	return 0;
err_miss_func_body:
	eprint(parser, "function body not found in definition");
err_free_self:
	free_yz_func(self);
	return 1;
}
