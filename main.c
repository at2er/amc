...
// int backend_init(struct mcb_context *context)
// {
// 	enum MCB_INIT_RESULT ret = mcb_init(context, backend_mode);
// 	switch (ret) {
// 	case MCB_INIT_SUCCESS:
// 		return 0;
// 	case MCB_INIT_FAILED_GET_MODE:
// 		printf(PANIC_FMT"failed to get backend mode\n", PANIC_FMT_ARG);
// 		return 1;
// 	case MCB_INIT_FAILED_TARGET_INIT:
// 		printf(PANIC_FMT"failed to init backend's target\n",
// 				PANIC_FMT_ARG);
// 		return 1;
// 	}
// 	printf(PANIC_FMT"failed to init backend\n",
// 			PANIC_FMT_ARG);
// 	return 1;
// }
...
int main(int argc, char *argv[])
{
	...
	struct yz_module root_mod;
	if (getarg(argc, argv, options))
		return 1;
	if (argc < 2)
		return print_version();
	// if (backend_init(&mcb_con))
	// 	return 1;
	yz_module_init(&root_mod);
	parser_init(&parser);
	parser.cur_mod = &root_mod;
